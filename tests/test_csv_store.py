...
class TestCSVStore:
    ...
    def test_update_from_health_check(self, csv_file, csv_store):
        ...
        assert proxies[PROXY_1].last_checked is not None, "PROXY_1 last_checked should be updated"
        ...
        assert proxies[PROXY_2].last_checked is not None, "PROXY_2 last_checked should be updated"
    ...
    def test_row_to_proxy_url(self):
        ...
        assert row1.to_proxy_url() == "http://192.168.1.1:8080", "HTTP URL should be formatted correctly"

        row2 = _Row(scheme="https", ip="192.168.1.2", port=443)
        assert row2.to_proxy_url() == "https://192.168.1.2:443", "HTTPS URL should be formatted correctly"

        row3 = _Row(scheme="socks5", ip="192.168.1.3", port=1080)
        assert row3.to_proxy_url() == "socks5://192.168.1.3:1080", "SOCKS5 URL should be formatted correctly"
    ...
    def test_malformed_proxy_data(self, csv_store):
        ...
        csv_store.upsert(PROXY_1, scheme="http", healthy=True, latency_ms=100, last_used=_utcnow())
        csv_store.upsert(PROXY_2, scheme="http", healthy=True, latency_ms="invalid", last_used=_utcnow())
        ...
        try:
            ...
            assert proxies[0].ip_port == "192.168.1.1:8080", "Expected valid proxy to be loaded"
        except Exception as e:
            assert False, f"Loading malformed data should not raise an exception, but got: {e}"